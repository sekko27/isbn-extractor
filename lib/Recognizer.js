...
class Recognizer {
    ...
    get isbn() {
        ...
        const startIx = this.length - 10;
        const chars = [];
        const ctx = {t: 10, s: 0};
        for (let ix = startIx; ix < this.length-1; ix++) {
            const b = this.buffer[ix];
            ctx.s += (ctx.t--) * b;
            chars.push(String.fromCharCode(CODE_0 + b))
        }
        const check = (11 - (ctx.s % 11)) % 11;
        (check === 10) ? chars.push('x') : chars.push(String.fromCharCode(CODE_0 + check));
        return chars.join('');
    }
